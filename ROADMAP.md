...
their licenses stay with them and Smelt keeps its own. Each capability degrades
...
- **v0.2 — Transcription:** video captions fetched via yt-dlp (json3); when a
  ...
  whisper.cpp, opt-in via `SMELT_WHISPER_MODEL`). Live-verified on Python 3.14.
- **v0.3 — Raw archive:** the evidence a note was made from — self-contained HTML
  ...
  Opt-in: `SMELT_ARCHIVE`, `SMELT_ARCHIVE_FORMATS`.
...
- **v0.6 — Visual extraction:** `ffmpeg` keyframes + `tesseract` OCR — what a
  video *shows*, not just what it says (opt-in `SMELT_VISUAL`).
...
- **Library:** SQLite + FTS5 index over everything digested — `smelt search`.
- **Ask your intake:** `smelt ask` answers strictly from your own notes, with
...
- **Batch intake:** `smelt ingest-batch` over a playlist or channel (bounded).
...
- **Learned relevance:** `smelt keep|drop <id>` teaches the router what this
  reader actually wants; the profile nudges confidence and can *propose* a drop.
  Dormant until enough verdicts exist (`smelt profile`).
- **Capture surface:** the friction that kills tools like this. Chrome extension
  ...
  PWA (`smelt serve --lan`, token-gated), bookmarklet — all pointing at your own
...
- **One source, one identity:** URLs are canonicalized before fetch (tracking
  ...
  source updates its note instead of duplicating it. `smelt dedupe` repairs older
  libraries.
