from verivann.adapters import archive
from verivann.adapters.archive import archive_all, archive_url, enabled, formats


def test_archive_disabled_by_default(monkeypatch, tmp_path):
    monkeypatch.delenv("VERIVANN_ARCHIVE", raising=False)
...
def test_archive_enabled_but_no_tool(monkeypatch, tmp_path):
    monkeypatch.setenv("VERIVANN_ARCHIVE", "1")
...
def test_formats_default_to_html_and_reject_junk(monkeypatch):
    monkeypatch.delenv("VERIVANN_ARCHIVE_FORMATS", raising=False)
    assert formats() == ["html"]
    monkeypatch.setenv("VERIVANN_ARCHIVE_FORMATS", "png, pdf ,exe")
    assert formats() == ["png", "pdf"]
    monkeypatch.setenv("VERIVANN_ARCHIVE_FORMATS", "nonsense")
    assert formats() == ["html"]


def test_archive_all_collects_every_available_format(monkeypatch, tmp_path):
    monkeypatch.setenv("VERIVANN_ARCHIVE", "1")
    monkeypatch.setenv("VERIVANN_ARCHIVE_FORMATS", "html,pdf,png")
...
def test_archive_all_skips_pdf_png_without_a_browser(monkeypatch, tmp_path):
    monkeypatch.setenv("VERIVANN_ARCHIVE", "1")
    monkeypatch.setenv("VERIVANN_ARCHIVE_FORMATS", "pdf,png")
