...
from verivann.mediapolicy import ALWAYS, ASK, FALLBACK, OFF, decide, keep_media, mode


@pytest.fixture(autouse=True)
def _clean(monkeypatch):
    monkeypatch.delenv("VERIVANN_MEDIA", raising=False)
    monkeypatch.delenv("VERIVANN_MEDIA_KEEP", raising=False)
...
def test_off_means_off_even_without_captions(monkeypatch):
    monkeypatch.setenv("VERIVANN_MEDIA", OFF)
    decision = decide(has_captions=False)
    assert decision.allowed is False
...
def test_the_policy_reaches_the_note(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "verivann.pipeline.extract_media",
