...
Two backends, both opt-in, neither of them a scraper:
    ...
    SearXNG   SMELT_SEARCH_URL=http://localhost:8888   — self-hosted, no key, no terms
              to violate, and the results never touch a third party's log.
    Brave     SMELT_SEARCH_KEY=...                     — a real API with a free tier.
...
def backend() -> str:
    if os.environ.get("SMELT_SEARCH_URL", "").strip():
        return "searxng"
    if os.environ.get("SMELT_SEARCH_KEY", "").strip():
        return "brave"
    return ""
...
def _searxng(query: str, limit: int) -> list[Result]:
    base = os.environ["SMELT_SEARCH_URL"].rstrip("/")
    resp = httpx.get(
        f"{base}/search",
...
def _brave(query: str, limit: int) -> list[Result]:
    resp = httpx.get(
        ...
        headers={
            ...
            "x-subscription-token": os.environ["SMELT_SEARCH_KEY"],
            "user-agent": _UA,
        },
        timeout=_TIMEOUT,
    )
