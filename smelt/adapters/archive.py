...
Opt-in (`SMELT_ARCHIVE` truthy). A note is an interpretation; the archive is the
...
Because these are separate programs, their copyleft licenses do not reach Smelt's
own license: the user installs them, Smelt orchestrates them.

Which formats are produced is `SMELT_ARCHIVE_FORMATS` (default "html"), e.g.
...
# Where a headless browser usually lives on Windows/macOS/Linux. SMELT_CHROME wins.
_CHROME_CANDIDATES = (
...
def enabled() -> bool:
    return os.environ.get("SMELT_ARCHIVE", "").strip().lower() in ("1", "true", "yes", "on")


def formats() -> list[str]:
    raw = os.environ.get("SMELT_ARCHIVE_FORMATS", _DEFAULT_FORMATS)
...
def chrome_binary() -> str | None:
    ...
    explicit = os.environ.get("SMELT_CHROME", "").strip()
    if explicit:
