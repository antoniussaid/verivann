...
class Config:
    ...
    @classmethod
    def load(cls) -> Config:
        staging = os.environ.get("SMELT_STAGING", "data/staging")
        domains = list(DEFAULT_PUBLIC_DOMAINS)
        public_demo = True

        # A private config file (path via env) may inject the real domains.
        private_path = os.environ.get("SMELT_PRIVATE_CONFIG")
        ...
        llm = _load_llm()
        try:
            price = float(os.environ.get("SMELT_PRICE_PER_MTOK", "0") or 0)
        except ValueError:
            price = 0.0
        return cls(
            ...
            llm=llm,
            local_llm=_load_llm("SMELT_LOCAL_LLM"),
            embed=_load_embed(llm),
            lens=os.environ.get("SMELT_LENS", DEFAULT_LENS).strip().lower() or DEFAULT_LENS,
...
def _preset_base(provider: str, default: str) -> str:
    ...
    if provider == "cloudflare":
        account = os.environ.get("SMELT_CF_ACCOUNT_ID", "").strip()
        return f"https://api.cloudflare.com/client/v4/accounts/{account}/ai/v1" if account else ""
    return default


def _load_llm(prefix: str = "SMELT_LLM") -> LLMConfig:
...
def _load_embed(llm: LLMConfig) -> EmbedConfig:
    ...
    model = os.environ.get("SMELT_EMBED_MODEL", "").strip()
    if not model:
        return EmbedConfig()  # disabled -> keyword search only
    fallback_base = "" if llm.provider == "anthropic" else llm.base_url
    return EmbedConfig(
        model=model,
        base_url=os.environ.get("SMELT_EMBED_BASE_URL", fallback_base),
        api_key=os.environ.get("SMELT_EMBED_API_KEY", llm.api_key),
    )
