...
app = typer.Typer(
    ...
    help="Smelt — raw web material -> structured, routed notes.",
...
def cockpit_default(ctx: typer.Context) -> None:
    """With no command, Smelt shows what is due — not a menu of thirty verbs."""
    ...
    if view.due_predictions:
        ...
        for p in view.due_predictions[:5]:
            ...
            typer.echo(f"         {p.source_label[:34]} · due {p.due} · smelt resolve {p.id} --hit|--miss")
    ...
    if view.changed_sources:
        ...
        for w in view.changed_sources[:3]:
            typer.echo(f"    {w['label'][:44]} ({w['changes']}×)  smelt watch run")
    ...
    if view.implied_drops or view.unjudged or view.stale_claims:
        typer.echo("\n  Waiting on you:")
        if view.unjudged:
            typer.echo(f"    {view.unjudged} note(s) never judged        smelt list")
        if view.implied_drops:
            typer.echo(f"    {view.implied_drops} note(s) ignored for 30+ days   smelt review")
        if view.stale_claims:
            typer.echo(f"    {view.stale_claims} claim(s) past their shelf life  smelt stale")
    if view.calibration:
...
_LENS_OPT = typer.Option(None, "--lens", "-l", help="Reading intent: see `smelt lenses`.")
...
def _run_import(candidates, limit: int, dry_run: bool, lens: str | None) -> None:
    ...
    typer.echo("  Now judge them — `smelt list`, then `smelt keep|drop <id>`.")
...
def question_list_cmd(
    ...
    if not rows:
        typer.echo("(no questions — `smelt question add \"...\"`)")
        return
    for q in rows:
        typer.echo(f"  {q.line()}\n")
...
def question_answer_cmd(question_id: int = typer.Argument(...)) -> None:
    ...
    if not result.answer:
        typer.echo("\n  (no LLM configured — evidence only. Set SMELT_LLM for the synthesis.)")
...
def feed_add_cmd(url: str = typer.Argument(..., help="RSS or Atom URL.")) -> None:
    """Follow a feed. Smelt reads it; you only see what gets through."""
...
def feed_list_cmd() -> None:
    ...
    rows = subscriptions()
    if not rows:
        typer.echo("(no feeds — `smelt feed add <url>`)")
        return
    for row in rows:
...
def feed_run_cmd(lens: str = _LENS_OPT) -> None:
    ...
    if mode == "untrained":
        ...
        typer.echo(
            "  Smelt has no basis for an editorial opinion yet, so it is not pretending to have one.\n"
            "  Give it one: `smelt keep|drop <id>` a few times, or register open questions\n"
            "  (`smelt question add \"...\"`) and configure a model."
        )
    else:
...
def lenses_cmd() -> None:
    ...
    typer.echo("\n  * default (SMELT_LENS overrides; --lens wins over both)")
...
def search_cmd(
    ...
    for h in found.hits:
        ...
        typer.echo("")
    if found.engine == "fts":
        typer.echo("  (keyword search only — set SMELT_EMBED_MODEL to also search by meaning)")
...
def reindex_cmd() -> None:
    ...
    cfg = Config.load()
    if not enabled(cfg):
        typer.echo("  no embedder configured — set SMELT_EMBED_MODEL (e.g. nomic-embed-text on Ollama).")
        raise typer.Exit(1)
...
def accept_cmd(
    ...
    to: Path = typer.Option(None, "--to", help="Target folder (or set SMELT_ACCEPT_DIR)."),
) -> None:
    ...
    if target is None:
        typer.echo("  where to? Pass --to <dir>, or set SMELT_ACCEPT_DIR.")
        raise typer.Exit(1)
...
def export_cmd(
    ...
    out: Path = typer.Option(Path("smelt-cards.txt"), "--out", help="Output file."),
) -> None:
...
def ask_cmd(question: str = typer.Argument(..., help="Ask your own digested notes.")) -> None:
    ...
    if not res.answer:
        typer.echo("\n(no LLM configured — matches only. Set SMELT_LLM to get an answer.)")
...
def watch_add(url: str = typer.Argument(..., help="URL to keep under observation.")) -> None:
    ...
    typer.echo("  baseline taken — `smelt watch run` will report what moves.")
...
def watch_list() -> None:
    ...
    rows = watched()
    if not rows:
        typer.echo("(nothing under observation — `smelt watch add <url>`)")
        return
    for w in rows:
...
def predictions_cmd(
    ...
    if overdue:
        typer.echo(f"  {overdue} prediction(s) have come due — judge them: smelt resolve <id> --hit|--miss")


@app.command("resolve")
def resolve_cmd(
    prediction_id: int = typer.Argument(..., help="Prediction id (see `smelt predictions`)."),
...
def _feedback(note_id: str, verdict: str) -> None:
    ...
    if hit is None:
        typer.echo(f"(no staged note with id starting '{note_id}' — see `smelt list`)")
        raise typer.Exit(1)
...
def review_cmd(
    ...
    if not sweep:
        typer.echo("  `smelt review --sweep` records them as dropped (weakly — a shrug, not a verdict).")
        typer.echo("  Rescue any of them with `smelt keep <id>`; that always wins over an inferred drop.")
        return
...
def counter_cmd(
    ...
    if not available():
        typer.echo("  no search backend — set SMELT_SEARCH_URL (SearXNG) or SMELT_SEARCH_KEY (Brave).")
        typer.echo("  Smelt does not scrape engines whose terms forbid it.")
        raise typer.Exit(1)
...
def daemon_cmd(
    ...
    typer.echo("  Smelt daemon — feeds, watchlist, living notes. Ctrl+C to stop.")
...
def reanalyze_cmd(
    ...
    if not (cfg.llm.enabled or cfg.local_llm.enabled):
        ...
        typer.echo("  `smelt models` shows the free options.")
        raise typer.Exit(1)
...
def bakeoff_cmd(
    ...
    if not results_only:
        ...
        if not cfg.llm.enabled:
            typer.echo("  no model endpoint configured — `smelt models` shows the free options.")
            raise typer.Exit(1)
        ...
        typer.echo(f"  Now judge them (`smelt list`, then keep/drop). {MIN_JUDGED} verdicts and the")
        typer.echo("  standings mean something: `smelt bakeoff --results`\n")
        return

    standings = results(cfg)
    if not standings.contenders:
        typer.echo("  no bake-off has been run — `smelt bakeoff --models a,b`")
        return
    typer.echo("")
...
def models_cmd() -> None:
    ...
    typer.echo("""
  Free ways to fill them (all OpenAI-compatible — Smelt speaks that everywhere)

    cloudflare   you already have an account if antonius.app runs on it.
                 SMELT_LLM=cloudflare
                 SMELT_CF_ACCOUNT_ID=<id>
                 SMELT_LLM_MODEL=@cf/meta/llama-3.3-70b-instruct-fp8-fast
                 SMELT_LLM_API_KEY=<token>
                 SMELT_EMBED_MODEL=@cf/baai/bge-base-en-v1.5

    groq         fast, generous free tier.       SMELT_LLM=groq
    openrouter   many models, some ':free'.      SMELT_LLM=openrouter
    gemini       free tier, OpenAI endpoint.     SMELT_LLM=gemini
    ollama       local, private, free forever.   SMELT_LLM=ollama

  Free tiers rate-limit. Give the chain somewhere to fall:
    SMELT_LLM_MODELS=<second model on the same endpoint>,<third>
    SMELT_LLM_FALLBACK=groq   (a whole other provider)

  Sensitive material (your files, IBANs, diagnoses) never goes to a hosted model
  unless you say so: SMELT_LOCAL_LLM=..., or SMELT_REDACT=1, or SMELT_PRIVACY=allow.
""")
...
def profile_cmd() -> None:
    ...
    if not profile.active:
        typer.echo(f"  status     : dormant (needs {MIN_SIGNAL} verdicts; use `smelt keep|drop <id>`)")
        return
...
def outbound_cmd(days: int = typer.Option(30, help="Look back this many days.")) -> None:
    ...
    if cfg.price_per_mtok:
        ...
        typer.echo(f"\n  {chars} characters sent. Set SMELT_PRICE_PER_MTOK to see what it cost.")
    typer.echo("")
...
def bias_cmd(topic: str = typer.Argument(..., help="A topic you think you understand.")) -> None:
    ...
    if split.engine == "inventory-only":
        typer.echo("  (no LLM configured — this is an inventory, not an audit. Set SMELT_LLM.)")
...
def doctor() -> None:
    ...
    cfg = Config.load()
    typer.echo(f"smelt {__version__}")
    ...
    transcription = f"whisper {os.environ.get('SMELT_WHISPER_MODEL', '')}" if whisper_on() else "captions only"
    if archive_on():
    ...
    from .privacy import policy
    typer.echo(
        f"  meaning search : {cfg.embed.model if embed_on(cfg) else 'off (keyword only — set SMELT_EMBED_MODEL)'}"
    )
    ...
    typer.echo(f"  counter-search : {search_backend() or 'off (set SMELT_SEARCH_URL or SMELT_SEARCH_KEY)'}")
    try:
        from .feeds import subscriptions
