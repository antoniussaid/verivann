...
We call ourselves local-first. Then we taught Smelt to read your screenshots, your
voice memos and your PDFs — and if `SMELT_LLM=openai` is set, every one of them is
...
And there are two model slots, not one:
    ...
    SMELT_LLM         may be hosted. Sees public material.
    SMELT_LOCAL_LLM   never leaves the machine (Ollama). Sees everything else.

Policy `SMELT_PRIVACY` (default **strict**):
...
def policy() -> str:
    value = os.environ.get("SMELT_PRIVACY", "strict").strip().lower()
...
def mode_for(config: Config, sensitivity: str) -> tuple[LLMConfig, bool, str]:
    ...
    Three ways sensitive material can be handled, and the user picks:
      ...
      redacted   SMELT_REDACT=1 — identity is stripped locally, the MEANING is sent
...
def explain(
    ...
    if mode == REDACTED:
        return (
            ...
            f"{redaction} No local model is configured; you enabled SMELT_REDACT to get this "
            "material read at all. `smelt outbound` records what was sent."
        )
    if mode == "withheld" or not llm.enabled:
        return (
            ...
            "(SMELT_LOCAL_LLM), enable local redaction (SMELT_REDACT=1), or accept the upload "
            "explicitly (SMELT_PRIVACY=allow)."
        )
    if stayed_local:
        return f"**Sensitive** ({why}). Analyzed by the local model — nothing left this machine."
    return (
        f"**Sensitive** ({why}) — and SMELT_PRIVACY=allow, so the full text was sent to a hosted "
        f"model ({llm.provider}:{llm.model}). This is on the record: `smelt outbound`."
    )
