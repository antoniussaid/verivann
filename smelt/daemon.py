...
editor you have to wake up every morning is not an editor.
    ...
    smelt daemon              run the schedule, forever (Ctrl+C to stop)
    smelt daemon --once       do one pass and exit — this is what a cron/Task
                              Scheduler entry should call
    smelt daemon --install    print the exact command for your OS's scheduler
...
* **It never asks anything.** `SMELT_MEDIA=ask` resolves to *no* in a background run,
...
def install_hint(python: str, staging: str) -> str:
  ...
  Windows (Task Scheduler) — every 30 minutes:
    ...
    schtasks /create /tn "Smelt" /sc minute /mo 30 ^
      /tr "{python} -m smelt.cli daemon --once"

  macOS / Linux (cron) — every 30 minutes:

    */30 * * * *  cd {staging} && {python} -m smelt.cli daemon --once
