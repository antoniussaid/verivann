...
another, and it is the one that touches a platform's terms. So Smelt never makes
that decision on its own, never makes it silently, and never makes it *for* the
user on someone else's machine.

    SMELT_MEDIA=off        never download media. Captions or nothing.
    SMELT_MEDIA=fallback   (default) download ONLY when the video has no captions —
                           and only to transcribe it. The file is deleted the moment
                           the transcript exists.
    SMELT_MEDIA=ask        decide per item.
    SMELT_MEDIA=always     download whenever it would help.

    SMELT_MEDIA_KEEP=1     keep the downloaded file (default: delete it immediately)
...
3. **The decision travels with the user, not with us.** Whatever runs on their
   machine, under their setting, is their act. Smelt does not download anything on
...
# plain text, behind no measure at all. So on these platforms Smelt reads the words
...
def mode() -> str:
    value = os.environ.get("SMELT_MEDIA", FALLBACK).strip().lower()
    return value if value in _MODES else FALLBACK


def keep_media() -> bool:
    """Keep the downloaded file? Off by default: we wanted the words, not the video."""
    return os.environ.get("SMELT_MEDIA_KEEP", "").strip().lower() in ("1", "true", "yes", "on")
...
def decide(has_captions: bool, asked=None, url: str = "") -> Decision:
    ...
    if protected(url):
        return Decision(
            ...
            "stream, so Smelt reads only the published caption track — which is plain text, "
...
def _did_download() -> str:
    if keep_media():
        return (
            "Audio was downloaded to transcribe it, **and kept** (SMELT_MEDIA_KEEP=1). "
