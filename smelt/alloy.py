...
The name promised this from the beginning and we never built it. A smelter does not
just purify one ore — it combines them, and the alloy is stronger than anything that
went in.

    smelt alloy <id> <id> [<id>…] --title "What ETFs actually cost in Austria"
...
def alloy(note_ids: list[str], title: str = "", config: Config | None = None) -> Alloy | None:
    ...
    event = IntakeEvent(
        ...
        provenance=Provenance(tool="smelt", version="0.1.0", public_demo=config.public_demo),
    )
