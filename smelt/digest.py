...
are the point.
    ...
    smelt digest              the last 7 days, to stdout
    smelt digest --days 30 --out week.md

Also here: `smelt export anki` — cards, but ONLY from notes you kept. A tool that
...
def digest(config: Config | None = None, days: int = 7) -> str:
    ...
    if due:
        ...
        for p in due:
            ...
            out.append(f"  - judge it: `smelt resolve {p.id} --hit|--miss`")
