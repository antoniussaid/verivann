...
to save a video. So the same server is also:
  ...
  * a share target — install it as an app on the phone (`smelt serve --lan`) and
    Smelt appears in the system share sheet;
...
<main>
  ...
  <footer>
    Capture without typing: drag <a id="bm" href="#">Smelt this</a> to your bookmarks bar &middot;
    install this page as an app to get Smelt in the share sheet.
  </footer>
</main>
...
<script>
  ...
  const TOKEN = params.get('token') || localStorage.getItem('smelt_token') || '';
  if (TOKEN) localStorage.setItem('smelt_token', TOKEN);
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code: int, body: bytes | str, ctype: str = "text/html; charset=utf-8") -> None:
        ...
        self.send_header("access-control-allow-headers", "content-type, x-smelt-token")
        self.end_headers()
    ...
    def _authorized(self) -> bool:
        if not self.token:
            return True
        given = self.headers.get("x-smelt-token", "")
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path in ("/", "/index.html"):
            if not self._authorized():
                return self._send(401, "unauthorized — open the URL that `smelt serve --lan` printed", "text/plain")
        ...
        if path == "/health":
            return self._json(200, {"ok": True, "tool": "smelt"})
...
def serve(
    ...
    print(f"Smelt inbox -> {local}")
    if exposed:
        print(f"  on your phone -> http://{_lan_ip()}:{port}/{suffix}")
        print("  (open it once, then 'Add to home screen' — Smelt joins the share sheet)")
