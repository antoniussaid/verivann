...
Everything in Smelt is a proposal. The router proposes a domain, the analyzer
...
`smelt accept <id> --to <dir>` is where you decide. It copies the staged note into
...
No lock-in: the destination is a folder of Markdown files. Smelt does not own it,
...
def default_target() -> Path | None:
    """`SMELT_ACCEPT_DIR` — so `smelt accept <id>` needs no flag in daily use."""
    value = os.environ.get("SMELT_ACCEPT_DIR", "").strip()
