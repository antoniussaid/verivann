...
note you read.
    ...
    SMELT_REDACT=1     analyze sensitive material after redaction (opt-in, explicit)
...
the honest advice — printed by the tool itself — is to keep `SMELT_PRIVACY=strict`
...
def enabled() -> bool:
    return os.environ.get("SMELT_REDACT", "").strip().lower() in ("1", "true", "yes", "on")
