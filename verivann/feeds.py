...
A feed reader shows you everything and calls the backlog your fault. Verivann reads
...
what got through**. The rest is staged, counted, and left alone.
    ...
    verivann feed add https://example.com/rss
    verivann feed run                  → "18 items, 3 got through"
...
def _gets_through(result, mode: str, profile) -> bool:
    ...
    There are three regimes, and conflating them is how a filter becomes a lie:
      ...
      untrained   Neither. Verivann has NO basis for an editorial opinion, so it does
