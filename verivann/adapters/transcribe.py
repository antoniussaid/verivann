...
def _model_name() -> str:
    return os.environ.get("VERIVANN_WHISPER_MODEL", "").strip()
...
def _via_whisper_cpp(audio: Path) -> tuple[str | None, str | None]:
    binary = os.environ.get("VERIVANN_WHISPER_CPP", "").strip()
    model = os.environ.get("VERIVANN_WHISPER_CPP_MODEL", "").strip()
