...
Verivann reads what you throw at it — articles, videos, screenshots, voice memos,
...
| | |
|---|---|
| **Measures itself** | Sources have a record. So does the router. `verivann calibrate` compares every proposal it ever made against your verdicts: *"it proposed keeping 40 notes; you kept 14 — and its confidence is not calibrated: being surer did not make it righter."* No other tool in this space will tell you how often it was wrong. |
| **Keeps score on your sources** | Every claim is filed under whoever made it. When a prediction fails or a claim is contradicted, that source carries it — permanently. `verivann sources` shows who has been right, who has been wrong, and whose 40-minute videos yield nothing. |
...
| **Keeps your disk off the cloud** | Anything from your own files — or carrying an IBAN, a salary, a diagnosis — is `sensitive`. Under the default policy it goes to a local model, or to nobody. It is never silently uploaded, and `verivann outbound` shows exactly what left and what it cost. |
| **Goes and finds the other side** | `verivann counter <id>` turns a note's weakest claims into counter-queries, fetches, and digests the results — so the claim ledger does the arguing. It never asks the accused for an alibi. |
| **Writes back into old notes** | `verivann refresh` — when a claim expires, a prediction fails, or a later source contradicts it, the old note gets a banner: *"Since you read this…"*. Obsidian notes are dead files; these are not. |
| **Puts predictions on the record** | "X will happen by Y" is stored with its due date and its author. On the date, Verivann asks you. `verivann predictions` · `verivann resolve <id> --miss` |
...
| **Shows you the mirror** | `verivann mirror` — hours consumed, notes produced, how many survived your own judgment, and which channel ate the time. There is no streak counter and no encouragement. |
| **Keeps the anti-library** | `verivann slag` — everything you threw away. The negative image of your taste. |
...
| **Watches for silent edits** | `verivann watch add <url>` → when the source is quietly rewritten, the note carries the diff. |
| **Routes to your questions, not to folders** | `verivann question add "..."` — every intake is read against what you are actually trying to find out. Material that advances none of it says so. `verivann question answer <id>` synthesizes everything you gathered, names where sources disagree, and states what is still missing. |
| **Reads your feeds for you** | `verivann feed run` — an editor, not a reader. Only what passes your learned filter surfaces. And when the filter is untrained, it says so instead of faking an opinion. |

## Core principles

- **Propose only, never commit.** The core never writes into a knowledge base. It
  emits a proposal (`note` / `task` / `drop`). `verivann accept <id> --to <dir>` is
...
verivann serve            # the inbox: paste, share or right-click a link
verivann doctor           # what is wired up, what is optional
```
...
| **Phone share sheet** | `verivann serve --lan` → open the printed URL on your phone → *Add to home screen*. Verivann now appears in the share sheet of every app. |
| **Bookmarklet** | Drag *Verivann this* from the inbox footer to your bookmarks bar. |
| **A folder** | `verivann folder ~/Pictures/Screenshots --watch` — the pile of screenshots you never open again, OCR'd and routed. Nothing is moved or deleted. |
...
`verivann dedupe` repairs older libraries.
...
```bash
verivann ingest-url "https://…" --lens critique
verivann lenses     # digest · wisdom · critique · study · actions
```
...
`verivann` on its own is the **cockpit**: what came due, what is rotting, what your
silence has already decided, and what the router itself got wrong. Everything else
is there when you want it.

```
verivann                                    what is due — the one screen
...
Verivann runs free and offline out of the box (heuristic routing, no model). Every
heavier capability is a program it *invokes*:

| Capability | Enable with | Needs |
|---|---|---|
| Analysis, lenses, claims, predictions, questions, bias | `VERIVANN_LLM=ollama\|openai\|anthropic` + `VERIVANN_LLM_MODEL` | a model — local (Ollama) or hosted |
| Meaning search (hybrid) | `VERIVANN_EMBED_MODEL=nomic-embed-text` | any `/embeddings` endpoint |
| Transcription (no captions / voice memos) | `VERIVANN_WHISPER_MODEL=base` | `faster-whisper` or `whisper.cpp` |
| Screenshots, on-screen text | (automatic / `VERIVANN_VISUAL=1`) | `tesseract`, `ffmpeg` |
| Raw archive (evidence) | `VERIVANN_ARCHIVE=1`, `VERIVANN_ARCHIVE_FORMATS=html,pdf,png` | `monolith`/`single-file`; any Chrome/Edge for PDF+PNG |
| PDFs | (automatic) | `pdftotext` or `pip install pypdf` |

`verivann doctor` tells you which of these are actually live. See `.env.example`.
